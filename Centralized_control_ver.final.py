...
def swmm_control(swmm_inputfile, orifice_id, basin_id, junction_id, time_step, csv_file_basename, controller1, num_ponds,
    ...
    with Simulation(swmm_inputfile) as sim:
        ...
        for j in range(num_ponds): # modify - num_ponds = 3
            sus.append(Nodes(sim)[f"{basin_id}{j+1}"]) #SU1,SU2,SU3
            water_depth_basins.append([])  # [],[],[]
            orifices.append(Links(sim)[f"{orifice_id}{j+1}"]) #OR1, OR2, OR3
            orifice_settings.append([]) #[],[],[]
            junctions.append(Nodes(sim)[f"{junction_id}{j+2}"]) #J2, J3, J4 # for stream
            water_depth_junctions.append([]) # for stream

        ca = Subcatchments(sim)["S1"]
        sim.step_advance(time_step)
        current_time = sim.start_time

        target_settings = get_control_strategy(sus, current_time, controller1,
                                                        period, horizon, rain_data_file,
                                                        weather_forecast_path, uncertainty)

        for j in range(num_ponds):
            orifices[j].target_setting = target_settings[j]
            orifice_settings[j].append(orifices[j].target_setting)
            water_depth_basins[j].append(sus[j].depth)
            water_depth_junctions[j].append(junctions[j].depth)
        ...
        for step in sim:
            ...
            #orifice.target_setting = get_control_strategy(sus[0].depth, current_time, controller,
            #                                                  period, horizon, rain_data_file,
            #                                                  weather_forecast_path, uncertainty)
            ...
            #                                                  period, horizon, rain_data_file,
            #                                                  weather_forecast_path, uncertainty)
            #orifice_settings[0].append(1.75*orifice.target_setting + 2)  # Offset for printing purposes
    ...
    with open(output_csv_file, "w") as f:
        ...
        for j in range(num_ponds): # num_ponds
        ...
        for line_index in range(len(time_series)):
            ...
            for water_depth_junction in water_depth_junctions:
...
def get_control_strategy(sus, current_time, controller, period, horizon,
    ...
    controller.controller.update_state({f'w1': sus[0].depth * 100}) #  Conversion from m to cm.
    controller.controller.update_state({f'w2': sus[1].depth * 100})  # Conversion from m to cm #add
    controller.controller.update_state({f'w3': sus[2].depth * 100})  # Conversion from m to cm #add
...
class StrategoController(sutil.SafeMPCSetup):
    """
...
class MPCSetupPond(sutil.SafeMPCSetup):
    ...
    def extract_control_action_from_stratego(self, stratego_output):
        ...
        """
        float_re = r"[-+]?(\d+(\.\d*)?|\.\d+)([eE][-+]?\d+)?"
        pattern1 = self.action_variable + r"1:\n\[0\]:( \(" + float_re + "," + float_re + r"\))*"
        pattern2 = self.action_variable + r"2:\n\[0\]:( \(" + float_re + "," + float_re + r"\))*"
        pattern3 = self.action_variable + r"3:\n\[0\]:( \(" + float_re + "," + float_re + r"\))*"

        result1 = re.search(pattern1, stratego_output)
        if result1 is None:
            raise RuntimeError(
                "Output of Stratego has not the expected format. Please check the output manually "
                "for error messages: \n" + stratego_output)

        float_tuples = get_float_tuples(result1.group())
        last_value1 = 0.0
        ...
        result2 = re.search(pattern2, stratego_output)
        if result2 is None:
            raise RuntimeError(
                "Output of Stratego has not the expected format. Please check the output manually "
                "for error messages: \n" + stratego_output)

        float_tuples = get_float_tuples(result2.group())
        last_value2 = 0.0
        ...
        for t in float_tuples:
            if t[0] == 0:
                ...
                break
        result3 = re.search(pattern1, stratego_output)
        if result3 is None:
            raise RuntimeError(
                "Output of Stratego has not the expected format. Please check the output manually "
                "for error messages: \n" + stratego_output)

        float_tuples = get_float_tuples(result3.group())
        last_value3 = 0.0
        ...
        for t in float_tuples:
            if t[0] == 0:
                ...
                break
        last_values = []
        last_values.append(last_value1)
        last_values.append(last_value2)
        last_values.append(last_value3)
    ...
    def create_query_file(self, horizon, period, final):
        ...
        with open(self.query_file, "w") as f:
            line1 = f"strategy opt = minE (2*st_c + w1 + w2 + w3) [<={horizon}*{period}]: <> (t=={final})\n" #c->st_c
            f.write(line1)
            f.write("\n")
...
def main():
    ...
    swmm_inputfile = os.path.join(base_folder, swmm_folder, "swmm1.inp")  # CLAIRE/swmm_models/swmm_decentralized.inp
    ...
    num_basins = 1 # in .inp file, only 3 ponds exits.
    time_step = 60 * 60  # 60 seconds/min x 60 min/h -> 1 h
    swmm_results = "swmm_centralized_results_final"
    num_ponds = 3
    ...
    controller1 = MPCSetupPond(model_template_path, output_file_path, query_file=query_file_path,
    ...
    swmm_control(swmm_inputfile, orifice_id, basin_id, junction_id, time_step, swmm_results, controller1, num_ponds,
                 period, horizon, rain_data_file, weather_forecast_path, uncertainty)
