...
def swmm_control(swmm_inputfile, orifice_id, basin_id, junction_id, time_step, csv_file_basename, controllers,
    ...
    with Simulation(swmm_inputfile) as sim:
        ...
        for j in range(len(controllers)):
            ...
            ponds.append(j+1)
            junctions.append(Nodes(sim)[f"{junction_id}{j+2}"]) #junction4 which reflects pond1,2,3 ## for stream
        ...
        for controller, orifice, orifice_setting, su, water_depth_basin, pond, junction in \
                zip(controllers, orifices, orifice_settings, sus, water_depth_basins, ponds, junctions):
            orifice.target_setting = get_control_strategy(su.depth, junction.depth, current_time, controller,
            ...
            water_depth_junctions.append(junction.depth)
    ...
    with open(output_csv_file, "w") as f:
        ...
        for line_index in range(len(time_series)):
            ...
            line.append(water_depth_junctions[line_index])
            writer.writerow(line)

def get_control_strategy(current_water_level, current_stream_level, current_time, controller, period, horizon,
                         rain_data_file, weather_forecast_path, uncertainty, pondid):
    global stream1
    global stream2
    global stream3
    global stream_count
    ...
    stream3 = float(0)  # new
    stream_count = 0 # new
    #stream = current_water_level
    # if stream_count >= 1:
    #    controller.controller.update_state({'st_w1': current_water_level * 100})  # Conversion from m to cm.

    if pondid == 1:
        controller.controller.update_state({'w': current_water_level * 100}) #  Conversion from m to cm.
        if stream_count == 0:
            stream1 = current_stream_level * 100
            controller.controller.update_state({'st_w1': current_stream_level * 100})  # Conversion from m to cm.
        else:
            stream1 = current_stream_level * 100
            controller.controller.update_state({'st_w1': stream3})  # Conversion from m to cm.
        #controller.controller.update_state({'st_w1': current_water_level * 50})  # Conversion from m to cm.
        #controller.controller.update_state({'st_w1': current_stream_level* 100})  # Conversion from m to cm.
        #stream_count = 1
        ...
        #print("\n")
        print("pond1: ", current_water_level*100)
        print("stream1: ", current_water_level * 50)

    elif pondid == 2:
        stream2 = current_stream_level * 100
        controller.controller.update_state({'w': current_water_level * 100}) #  Conversion from m to cm.
        controller.controller.update_state({'st_w2': stream1})  # Conversion from m to cm.
        #stream2 = stream1
        #controller.controller.update_state({'st_w2': stream2})  # Conversion from m to cm.
        control_setting = controller.run_single(period, horizon, start_date=current_time,
                                                historical_rain_data_path=rain_data_file,
                                                weather_forecast_path=weather_forecast_path,
                                                uncertainty=uncertainty)
        print("pond2: ", current_water_level*100)
        print("stream2: ", current_water_level * 50)

    elif pondid == 3:
        stream3 = current_stream_level * 100
        controller.controller.update_state({'w': current_water_level * 100})  # Conversion from m to cm.
        controller.controller.update_state({'st_w3': stream2})  # Conversion from m to cm.
        control_setting = controller.run_single(period, horizon, start_date=current_time,
                                                historical_rain_data_path=rain_data_file,
                                                weather_forecast_path=weather_forecast_path,
                                                uncertainty=uncertainty)
        print("pond3: ", current_water_level*100)
        print("stream3: ", current_water_level * 50)
...
def main():
    ...
    swmm_results = "swmm_compo_results"
