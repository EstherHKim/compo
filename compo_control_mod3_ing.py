...
def get_control_strategy(current_water_level, current_time, controller, period, horizon,
    ...
    #global stream = 0.0 # new
    #global stream_count = 0 # new
    ...
    if pondid == 1:
        ...
        #print("\n")
        print("pond1", current_water_level)

    elif pondid == 2:
        controller.controller.update_state({'w': current_water_level * 100}) #  Conversion from m to cm.
        controller.controller.update_state({'st_w2': current_water_level * 50})  # Conversion from m to cm.
        control_setting = controller.run_single(period, horizon, start_date=current_time,
                                                historical_rain_data_path=rain_data_file,
                                                weather_forecast_path=weather_forecast_path,
                                                uncertainty=uncertainty)
        print("pond2", current_water_level)

    elif pondid == 3:
        controller.controller.update_state({'w': current_water_level * 100}) #  Conversion from m to cm.
        controller.controller.update_state({'st_w3': current_water_level * 50})  # Conversion from m to cm.
        control_setting = controller.run_single(period, horizon, start_date=current_time,
                                                historical_rain_data_path=rain_data_file,
                                                weather_forecast_path=weather_forecast_path,
                                                uncertainty=uncertainty)
        print("pond3", current_water_level)
