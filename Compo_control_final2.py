...
# expand pond numbers compare to previous version.
# 1 pond => 2 ponds
...
def swmm_control(swmm_inputfile, orifice_id, basin_id, junction_id, time_step, csv_file_basename, controllers,
    ...
    with Simulation(swmm_inputfile) as sim:
        ...
        #for controller, orifice, orifice_setting, su, water_depth_basin, junction, water_depth_junction in \
        #        zip(controllers, orifices, orifice_settings, sus, water_depth_basins, junctions, water_depth_junctions):
        for controller, orifice, orifice_setting, su, water_depth_basin, pond in \
               zip(controllers, orifices, orifice_settings, sus, water_depth_basins, ponds):

            orifice.target_setting = get_control_strategy(su.depth, current_time, controller,
                                                             period, horizon, rain_data_file,
                                                             weather_forecast_path, uncertainty, pond)
...
def get_control_strategy(current_water_level, current_time, controller, period, horizon,
                         rain_data_file, weather_forecast_path, uncertainty, pond_id):
    if pond_id == 1:
        controller.controller.update_state({'w1': current_water_level * 100}) #  Conversion from m to cm.
        control_setting = controller.run_single(period, horizon, start_date=current_time,
                                            ...
                                            uncertainty=uncertainty, pond_id=pond_id)
    elif pond_id == 2:
        controller.controller.update_state({'w2': current_water_level * 100})  # Conversion from m to cm.
        control_setting = controller.run_single(period, horizon, start_date=current_time,
                                                historical_rain_data_path=rain_data_file,
                                                weather_forecast_path=weather_forecast_path,
                                                uncertainty=uncertainty, pond_id=pond_id)
    #print(control_setting) # seems like control_setting is not exist!
...
class MPCSetupPond(sutil.SafeMPCSetup):
    ...
    def __init__(self, model_template_file, output_file_path=None, query_file="",
                 ...
                 external_simulator=False, action_variable=None, debug=False, pond_id=1):
    ...
    def run_single(self, control_period, horizon, pond_id, **kwargs):
        """
        ...
        chosen_action = self.extract_control_action_from_stratego(pond_id, result)
    ...
    def extract_control_action_from_stratego(self, pond_id, stratego_output):
        """
        ...
        pattern = self.action_variable + str(pond_id) + r":\n\[0\]:( \(" + float_re + "," + float_re + r"\))*"
        result = re.search(pattern, stratego_output)

        #print(self.action_variable + pond_id)
        if result is None:
            raise RuntimeError(
    ...
    def create_query_file(self, horizon, period, final):
        ...
        with open(self.query_file, "w") as f:
            line1 = f"strategy opt = minE (2*st_c1 + c1) [<={horizon}*{period}]: <> (t=={final})\n"
            f.write(line1)
            f.write("\n")
    ...
    def perform_at_start_iteration(self, control_period, horizon, duration, step, **kwargs):
        """
        ...
        weather.create_weather_forecast(kwargs["historical_rain_data_path"],
                                        ...
                                        horizon * control_period, kwargs["uncertainty"])
...
def main():
    ...
    num_basins = 2 # 3 ponds
    time_step = 60 * 60  # 60 seconds/min x 60 min/h -> 1 h
    swmm_results = "swmm_compo_results_2ponds"
    ...
    # Path for common used files
    learning_config_path = os.path.join(uppaal_folder, "verifyta_compo_stream_config.yaml")
    weather_forecast_path = os.path.join(uppaal_folder, "compo_weather_forecast.csv")
    verifyta_command = "verifyta-stratego-10"

    # Path of files related to Pond1
    model_template_path_1 = os.path.join(uppaal_folder, "pond1_compo_stream.xml") #here
    query_file_path_1 = os.path.join(uppaal_folder, "pond1_compo_stream_query.q")
    model_config_path_1 = os.path.join(uppaal_folder, "pond1_compo_stream_config.yaml")
    output_file_path_1 = os.path.join(uppaal_folder, "pond1_compo_stream_result.txt")
    insert_paths_in_uppaal_model(model_template_path_1, weather_forecast_path,
                                 os.path.join(uppaal_folder, "libtable.so"))

    # Path of files related to Pond2
    model_template_path_2 = os.path.join(uppaal_folder, "pond2_compo_stream.xml")  # here
    query_file_path_2 = os.path.join(uppaal_folder, "pond2_compo_stream_query.q")
    model_config_path_2 = os.path.join(uppaal_folder, "pond2_compo_stream_config.yaml")
    output_file_path_2 = os.path.join(uppaal_folder, "pond2_compo_stream_result.txt")
    insert_paths_in_uppaal_model(model_template_path_2, weather_forecast_path,
                                 os.path.join(uppaal_folder, "libtable.so"))

    # Path of files related to Pond3
    """ 
    model_template_path_3 = os.path.join(uppaal_folder, "pond3_compo_stream.xml")  # here
    query_file_path_3 = os.path.join(uppaal_folder, "pond3_compo_stream_query.q")
    model_config_path_3 = os.path.join(uppaal_folder, "pond3_compo_stream_config.yaml")
    output_file_path_3 = os.path.join(uppaal_folder, "pond3_compo_stream_result.txt")
    insert_paths_in_uppaal_model(model_template_path_3, weather_forecast_path,
                                 os.path.join(uppaal_folder, "libtable.so"))
    """

    # Define uppaal model's action strategy variables.
    action_variable = "Open"  # Name of the control variable
    action_variable_1 = "Open1"  # Name of the control variable
    # action_variable_2 = "Open2"  # Name of the control variable
    # action_variable_3 = "Open3"  # Name of the control variable

    # Define parameters for online synthesis.
    ...
    controllers = []
    controller = MPCSetupPond(model_template_path_1, output_file_path_1, query_file=query_file_path_1,
                                model_cfg_dict=model_cfg_dict_1,
                                learning_args=learning_cfg_dict,
                                verifyta_command=verifyta_command,
                                external_simulator=False,
                                action_variable=action_variable, debug=debug, pond_id=1)
    controllers.append(controller)

    controller = MPCSetupPond(model_template_path_2, output_file_path_2, query_file=query_file_path_2,
                              model_cfg_dict=model_cfg_dict_2,
                              learning_args=learning_cfg_dict,
                              verifyta_command=verifyta_command,
                              external_simulator=False,
                              action_variable=action_variable, debug=debug, pond_id=2)
    controllers.append(controller)

    #controller = MPCSetupPond(model_template_path_3, output_file_path_3, query_file=query_file_path_3,
    #                          model_cfg_dict=model_cfg_dict_3,
    #                          learning_args=learning_cfg_dict,
    #                          verifyta_command=verifyta_command,
    #                          external_simulator=False,
    #                          action_variable=action_variable_3, debug=debug, pond_id=3)
    #controllers.append(controller)
