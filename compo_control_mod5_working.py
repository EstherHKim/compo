...
def get_control_strategy(current_water_level, current_stream_level, current_time, controller, period, horizon,
    ...
    # current_water_level and current_stream_level from SWMM.
    global stream1
    global stream2
    global stream3
    global stream_count
    ...
    if pondid == 1:
        ...
        controller.controller.update_state({'w': current_water_level * 100}) #  Conversion from m to cm.
        #controller.controller.update_state({'st_w2': current_stream_level * 100})  # Conversion from m to cm.
        #stream2 = stream1
        ...
        controller.controller.update_state({'w': current_water_level * 100})  # Conversion from m to cm.
        #controller.controller.update_state({'st_w3': current_stream_level * 100})  # Conversion from m to cm.
        control_setting = controller.run_single(period, horizon, start_date=current_time,
                                                historical_rain_data_path=rain_data_file,
                                                weather_forecast_path=weather_forecast_path,
                                                uncertainty=uncertainty)
...
class MPCSetupPond(sutil.SafeMPCSetup):
    ...
    def create_query_file(self, horizon, period, final):
        ...
        with open(self.query_file, "w") as f:
            line1 = f"strategy opt = minE (st_c + c) [<={horizon}*{period}]: <> (t=={final})\n"
            f.write(line1)
            f.write("\n")
...
def main():
    ...
    swmm_inputfile = os.path.join(base_folder, swmm_folder, "swmm2.inp")
